...
namespace Enki
{
	...
	ObjectSensor::ObjectSensor(Robot *owner, Vector pos, double height, double orientation, double range, double m, double x0, double c, double noiseSd):
		pos(pos),
		height(height),
	...
	void ObjectSensor::wallsStep (double dt, World* w)
	{
		switch (w->wallsType)
		{
	...
	void ObjectSensor::finalize(double dt, World* w)
	{
	...
	bool ObjectSensor::updateRay(size_t i, double dist)
	{
        bool updated(false);
	...
	double ObjectSensor::responseFunction(double x) const
	{
	...
	double ObjectSensor::inverseResponseFunction(double v) const
	{
		assert(v >= 0);
		assert(v <= m);
		if (v == 0)
	...
	double ObjectSensor::distanceToPolygon(double rayAngle, const Polygone &p) const 
	{
